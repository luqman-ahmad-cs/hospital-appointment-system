...
$doctors = mysqli_query($conn,
    "SELECT u.id, u.fullname, u.email, u.phone, u.status, u.created_at,
...
<div class="main-content">
    ...
    <div class="table-card">
        <h5><i class="fas fa-user-md text-primary"></i> 
            All Doctors 
            <span style="font-size:14px;color:#888;font-weight:400">
                (<?php echo mysqli_num_rows($doctors); ?> total)
            </span>
        </h5>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <?php $i = 1; while ($row = mysqli_fetch_assoc($doctors)): ?>
                    <tr>
                        <td><?php echo $i++; ?></td>
                        <td><strong>Dr. <?php echo $row['fullname']; ?></strong></td>
                        ...
                        <td>
                            <?php if($row['status'] == 'pending'): ?>
                                <a href="doctor_action.php?id=<?php echo $row['id']; ?>&action=approve"
                                   class="btn-action btn-approve">✅ Approve</a>
                            <?php endif; ?>
                            <?php if($row['status'] == 'active'): ?>
                                <a href="doctor_action.php?id=<?php echo $row['id']; ?>&action=block"
                                   class="btn-action btn-block">🚫 Block</a>
                            <?php endif; ?>
                            <?php if($row['status'] == 'blocked'): ?>
                                <a href="doctor_action.php?id=<?php echo $row['id']; ?>&action=unblock"
                                   class="btn-action btn-unblock">✅ Unblock</a>
                            <?php endif; ?>
                            <a href="doctor_action.php?id=<?php echo $row['id']; ?>&action=delete"
